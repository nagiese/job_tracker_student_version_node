...
module.exports = {
    ...
    accountlist: function(cb){
        var list = '<label for="accountList">Select an Account</label><br><select class="custom-select" id="accountList"><option value="0">Select an Account</option>', i = 0, sql;
    ...
    contactlist: function(cb){
        var list = '<label for="contactList">Select a Contact</label><br><select class="custom-select" id="contactList"><option value="0">Select a Contact</option>', i = 0, sql;
    ...
    joblist: function(acctId, cb){
        var sql, values, i=0, list = '<label for="jobList">Select a Job</label><br><select class="custom-select" id="jobList"><option value="0">Select a Job</option>';
    ...
    checkDuplicates: function(elements, flag, acctId, cb){
        var i = 0, res, sql, values, returnedValue;
                ...
                if(validate.validate(elements[i].value, elements[i].regex)){
                    ...
                    /* IF THE FLAG IS AN UPDATE THEN FIRST CHECK IF THE NAME IS THE SAME AS THE ORIGINAL NAME.  IF SO THEN GO NO FURTHER.  IF NOT THEN CHECK FOR DUPLICATE NAME IN DATABASE */
                        ...
                        switch (elements[i].duplicates){
                            case 'account' : sql = "SELECT ?? FROM ?? WHERE id = ?"; break;
                            case 'contact' : sql = "SELECT ?? FROM ?? WHERE id = ?"; break;
                            case 'job' : sql = "SELECT ?? FROM ?? WHERE id = ?"; break;
                        }
...
function duplicates(element, cb){
    ...
    switch (element.duplicates){
        case 'account' : sql = "SELECT ?? FROM ?? WHERE name = ?"; break;
        case 'contact' : sql = "SELECT ?? FROM ?? WHERE email = ?";break;
        case 'job' : sql = "SELECT ?? FROM ?? WHERE name = ?"; break;
    }
